...
async function seedTestUsers() {
  ...
  const ownerRes = await db.query("SELECT id FROM users WHERE phone = '01097132814'");
  if (ownerRes.rows.length > 0) {
    await db.query("UPDATE users SET password_hash = $1 WHERE phone = '01097132814'", [ownerHash]);
    console.log('Owner 01097132814 password updated to changeme123');
  } else {
    await db.query(
      "INSERT INTO users (name, phone, email, password_hash, role, is_active) VALUES ($1, $2, $3, $4, 'owner', true)",
      ['Shop Owner', '01097132814', 'owner@deliveryhub.local', ownerHash]
    );
    console.log('Owner 01097132814 created');
  }
